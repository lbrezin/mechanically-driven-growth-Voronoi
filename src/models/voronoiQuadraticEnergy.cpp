...
*/
VoronoiQuadraticEnergy::VoronoiQuadraticEnergy(int n, Dscalar scale, bool reprod)
    {
    printf("Initializing %i cells with random positions in a square box... \n",n);
    Reproducible = reprod;
    initializeVoronoiQuadraticEnergy(n, scale);
...
*/
VoronoiQuadraticEnergy::VoronoiQuadraticEnergy(int n,Dscalar scale, Dscalar A0, Dscalar P0,bool reprod)
    {
    printf("Initializing %i cells with random positions in a square box...\n ",n);
    Reproducible = reprod;
    initializeVoronoiQuadraticEnergy(n, scale);
...
void VoronoiQuadraticEnergy::initializeVoronoiQuadraticEnergy(int n, Dscalar scale)
    {
    initializeVoronoiModelBase(n, scale);
    Timestep = 0;
    setDeltaT(0.01);
    };
...
void VoronoiQuadraticEnergy::computeVoronoiForceSetsGPU()
    ...
    //Add the handle for Moduli
    ArrayHandle<Dscalar2> d_m(Moduli,access_location::device,access_mode::read);
...
void VoronoiQuadraticEnergy::computeVoronoiForceCPU(int i)
    ...
    Dscalar Adiff = h_m.data[i].x*(h_AP.data[i].x - h_APpref.data[i].x);
    Dscalar Pdiff = h_m.data[i].y*(h_AP.data[i].y - h_APpref.data[i].y);
    ...
    for(int nn = 0; nn < neigh; ++nn)
        ...
        Dscalar Akdiff = h_m.data[i].x*(h_AP.data[baseNeigh].x  - h_APpref.data[baseNeigh].x);
        Dscalar Pkdiff = h_m.data[i].y*(h_AP.data[baseNeigh].y  - h_APpref.data[baseNeigh].y);
        Dscalar Ajdiff = h_m.data[i].x*(h_AP.data[otherNeigh].x - h_APpref.data[otherNeigh].x);
        Dscalar Pjdiff = h_m.data[i].y*(h_AP.data[otherNeigh].y - h_APpref.data[otherNeigh].y);
...
Matrix2x2 VoronoiQuadraticEnergy::d2Edridrj(int i, int j, neighborType neighbor,Dscalar unstress, Dscalar stress)
    ...
    Dscalar dEdA = 2*h_m.data[i].x*(h_AP.data[i].x - h_APpref.data[i].x);
    Dscalar dEdP = 2*h_m.data[i].y*(h_AP.data[i].y - h_APpref.data[i].y);
    ...
    answer += area*unstress*2.0*h_m.data[i].x*dyad(dAidrj(i,i),dAadrj);
    answer += peri*unstress*2.0*h_m.data[i].y*dyad(dPidrj(i,i),dPadrj);
    for (int vv = 0; vv < neigh; ++vv)
        ...
        //cell gamma terms
        Dscalar dEGdP = 2.0*h_m.data[i].y*(h_AP.data[cellG].y - h_APpref.data[cellG].y);
        Dscalar dEGdA = 2.0*h_m.data[i].x*(h_AP.data[cellG].x  - h_APpref.data[cellG].x);
        //area part
        Dscalar2 dAGdv;
        ...
        //first term
        answer += area*unstress*2.*h_m.data[i].x*dyad(dAGdv*dvidri,dAGdrj);
        //second term
        ...
        answer += peri*unstress*2.*h_m.data[i].y*dyad(dPGdv*dvidri,dPGdrj);
        //second term
        ...
        //cell beta terms
        Dscalar dEBdP = 2.0*h_m.data[i].y*(h_AP.data[cellB].y - h_APpref.data[cellB].y);
        Dscalar dEBdA = 2.0*h_m.data[i].x*(h_AP.data[cellB].x - h_APpref.data[cellB].x);
        //
        //area terms
        Dscalar2 dABdv;
        ...
        //first term
        answer += area*unstress*2.*h_m.data[i].x*dyad(dABdv*dvidri,dABdrj);
        //second term
        ...
        answer += peri*unstress*2.*h_m.data[i].y*dyad(dPBdv*dvidri,dPBdrj);
        //second term
