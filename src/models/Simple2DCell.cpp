...
*/
void Simple2DCell::initializeSimple2DCell(int n, Dscalar scale)
    {
    Ncells = n;
    Nvertices = 2*Ncells;

    //setting cell positions randomly also auto-generates a square box with L = sqrt(Ncells)
    setCellPositionsRandomly(scale);
...
*/
void Simple2DCell::setCellPositionsRandomly(Dscalar scale)
    {
    cellPositions.resize(Ncells);
    Dscalar boxsize = scale*sqrt((Dscalar)Ncells);
...
void Simple2DCell::setModuli(vector<Dscalar> &newKA, vector<Dscalar> &newKP)
    {
    Moduli.resize(Ncells);
    ArrayHandle<Dscalar2> h_m(Moduli,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
        h_m.data[ii].x = newKA[ii];
        h_m.data[ii].y = newKP[ii];
        };
    };
...
/*
// Set all cell lineages to i
void Simple2DCell::setCellLineageUniform(int i)
    {
    cellLineage.resize(Ncells);
    ArrayHandle<int> h_cl(cellLineage,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
        h_cl.data[ii] = i;
        };
...
/*
// Lineages a vector of integers that the cell lineages will be set to
void Simple2DCell::setCellLineage(vector<int> &lineages)
    {
    cellLineage.resize(Ncells);
    ArrayHandle<int> h_cl(cellLineage,access_location::host,access_mode::overwrite);
    for (int ii = 0; ii < Ncells; ++ii)
        {
        h_cl.data[ii] = lineages[ii];
        };
    };
*/
...
void Simple2DCell::cellDeath(int cellIndex)
    ...
    //removeGPUArrayElement(cellLineage,cellIndex);
...
void Simple2DCell::cellDivision(const vector<int> &parameters, const vector<Dscalar> &dParams)
    ...
    growGPUArray(cellPositions,1);
        ...
        //ArrayHandle<int> h_cl(cellLineage); h_cl.data[Ncells-1] = h_cl.data[cellIdx];
...
vector<Dscalar> Simple2DCell::computePressure()
{
    vector<Dscalar> cellPressure(Ncells);
    //Calls for area, perimeter and elasticities
    ArrayHandle<Dscalar2> h_AP(AreaPeri,access_location::host,access_mode::read);
    ArrayHandle<Dscalar2> h_APP(AreaPeriPreferences,access_location::host,access_mode::read);
   
    //Calls related to the neighbors
    ...
    ArrayHandle<Dscalar2> h_moduli(returnModuli(),access_location::host,access_mode::read);
    for (int cell = 0; cell < Ncells; ++cell)
    {
        ...
        //Gets the neighbors of the cells
        for (int nn = 0; nn < neigh; ++nn)
        {
            ns[nn] = h_n.data[n_idx(nn,cell)];
            voro[nn] = h_v.data[n_idx(nn,cell)];
        };
        //Now sum up the normal forces on the edges
        Dscalar laplace=0; //variable to compute the laplace pressure
        Dscalar KAself = h_moduli.data[cell].x;
        Dscalar KPself = h_moduli.data[cell].y;
        Dscalar AreaSelf = h_AP.data[cell].x;
        Dscalar Area0Self = h_APP.data[cell].x;
        Dscalar2 vcur, vnext, dnext;
        for (int nn = 0; nn < neigh; ++nn)
        {
            vcur = voro[nn]; //current vertex
            vnext = voro[(nn+1)%neigh];//next vertex Counter clock wise
            int baseNeigh = ns[nn];
            //KPneigh = h_moduli.data[baseNeigh].y;
            ...
            laplace += (KPself*(h_AP.data[cell].y-h_APP.data[cell].y)+h_moduli.data[baseNeigh].y*(h_AP.data[baseNeigh].y-h_APP.data[baseNeigh].y))*dnnorm;
        }
        cellPressure[cell] = -KAself*(AreaSelf-Area0Self)-1/(4*AreaSelf)*laplace;
    }
    return cellPressure;
}

vector<Dscalar> Simple2DCell::computePeri()
{
    vector<Dscalar> cellPeri(Ncells);
    //Calls for area, perimeter and elasticities
    ArrayHandle<Dscalar2> h_AP(AreaPeri,access_location::host,access_mode::read);
    ArrayHandle<Dscalar2> h_APP(AreaPeriPreferences,access_location::host,access_mode::read);
    ...
    for (int cell = 0; cell < Ncells; ++cell)
    {
        ...
        //Now sum up the normal forces on the edges
        Dscalar peri=0; //variable to compute the laplace pressure
        Dscalar2 vcur, vnext, dnext;
        for (int nn = 0; nn < neigh; ++nn)
        {
            vcur = voro[nn]; //current vertex
...
//Computes the rate of each cell depending on its area ar, area preference arpref, its selective advantage adv,
//and the type of rate. Index=0 for sigmoid, 1 for exponential and 2 for
//linear 
//Dscalar Simple2DCell::birthRate(Dscalar ar, Dscalar arpref, Dscalar adv, Dscalar scale, int index)
//{
//    Dscalar rate;
//    switch(index)
//    {
//        case 0:
//            rate = 1+erf(scale*(ar-arpref))+adv ;
//            break;
//        case 1:
//            rate = exp(ar-arpref);
//            break;
//        case 2:
//            if(ar-arpref<0){rate = 0;}
//            else {rate = ar-arpref;}
//            break;
//    }
//    return rate;
//}
//
//Dscalar Simple2DCell::deathRate(Dscalar ar, Dscalar arpref, Dscalar adv, Dscalar scale, int index)
//{
//    Dscalar rate;
//    switch(index)
//    {
